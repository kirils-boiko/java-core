...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        for (String s : data) {
            StringItem item = new StringItem(s, null, null);
            linkedList.add(item);
        }

        printLinkedList(linkedList);

        Item toRemove = linkedList.get(3);   // middle of the list
        linkedList.remove(toRemove);
        printLinkedList(linkedList);

        toRemove = linkedList.get(0);       // first element
        linkedList.remove(toRemove);
        printLinkedList(linkedList);

        toRemove = linkedList.get(0);       // new first element
        linkedList.remove(toRemove);
        printLinkedList(linkedList);

        toRemove = linkedList.get(3);       // last element
        linkedList.remove(toRemove);
        printLinkedList(linkedList);

        toRemove = linkedList.get(3);       // out of range
        linkedList.remove(toRemove);
        printLinkedList(linkedList);
